...
async def get_advertisment(session, ad_id):
    url = f'{BASE_URL}/advertisment/{ad_id}/'
    async with session.get(url) as response:
        if response.status == 200:
            advertisment = await response.json()
            print(f"Advertisment details: {advertisment}")
        else:
            error = await response.json()
            print(f"Error getting advertisment: {error}")


async def update_advertisment(session, ad_id, new_data):
    url = f'{BASE_URL}/advertisment/{ad_id}/'
    async with session.patch(url, json=new_data) as response:
        if response.status == 200:
            advertisment = await response.json()
            print(f"Advertisment updated: {advertisment}")
        else:
            error = await response.json()
            print(f"Error updating advertisment: {error}")


async def delete_advertisment(session, ad_id):
    url = f'{BASE_URL}/advertisment/{ad_id}/'
    async with session.delete(url) as response:
        if response.status == 200:
            print(f"Advertisment {ad_id} deleted")
        else:
            error = await response.json()
            print(f"Error deleting advertisment: {error}")
...
async def main():
    async with aiohttp.ClientSession() as session:
        ...
        # Получаем объявление
        await get_advertisment(session, ad_id=1)

        # Обновляем объявление
        await update_advertisment(session, ad_id=1, new_data={"title": "Updated Car", "description": "Updated description"})

        # Удаляем объявление
        await delete_advertisment(session, ad_id=1)
